           ...
           public void run() {
               try {
                    ...
                    if (Eln.analyticsPlayerUUIDOptIn && FMLCommonHandler.instance().getEffectiveSide()== Side.CLIENT) {
                        ...
                        String formatUrl = "%s?version=%s&lang=%s&uuid=%s&cableFactor=%s&cableResistanceMultiplier=%s&explosions=%s&repOn=%s";
                        url = String.format(formatUrl, Eln.analyticsURL, version, lang, Eln.playerUUID, Eln.cableFactor, Eln.cableResistanceMultiplier, Eln.explosionEnable, Eln.replicatorPop);
                    }
