    ...
    inner class FlyWheelFlingProcess : IProcess {
        ...
        fun slowProcess() {
            ...
            for(obj in objects) {
                ...
                DP.println(DPType.MECHANICAL, String.format("FPP.sP: considering %s", ent))
                val dx = Math.abs(ent.posX - coord.x - 0.5)
                val dy = Math.abs(ent.posY - coord.y - 1)
                val dz = Math.abs(ent.posZ - coord.z - 0.5)
                if(dy > yTolerance) {
                    DP.println(DPType.MECHANICAL, "FPP.sP: dy out of range (" + dy + "; c.y " + coord.y + " e.y" + ent.posY + "): " + ent)
                    continue
                }
                if(dx > xzTolerance) {
                    DP.println(DPType.MECHANICAL, "FPP.sP: dx out of range (" + dx + "; c.x " + coord.x + " e.x" + ent.posX + "): " + ent)
                    continue
                }
                if(dz > xzTolerance) {
                    DP.println(DPType.MECHANICAL, "FPP.sP: dz out of range (" + dz + "; c.z " + coord.z + " e.z" + ent.posZ + "): " + ent)
                    continue
                }
                ...
                DP.println(DPType.MECHANICAL, "FFP.sP: ent " + ent + " flung " + vel.joinToString(",") + " for damage " + dmg)
                if(dmg <= 0) continue
