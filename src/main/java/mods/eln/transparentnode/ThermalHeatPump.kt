package mods.eln.transparentnode.themralheatpump
...
//It's a carbon copy of the Heat Exchanger only it will heat up or cool down a fluid using electricity, with the opposite temperature coming out the thermal port
//Basically it brings back a glitch from the original heat exchanger as a mechanic, because it was really fun and made geothermal possible.
//Think of it like the Active Thermal Dissapator of the Heat Exchanger
class ThermalHeatPumpDescriptor(
    name: String,
    val thermal: ThermalLoadInitializerByPowerDrop
): TransparentNodeDescriptor(
    name,
    ThermalHeatPumpElement::class.java,
    ThermalHeatPumpRender::class.java,
    EntityMetaTag.Fluid
) {
...
class ThermalHeatPumpElement(
    ...
    init {
        ...
        thermalWatchdog.set(thermalLoad).setLimit((descriptor as ThermalHeatPumpDescriptor).thermal)
            .set(WorldExplosion(this).machineExplosion())

        if (ic2hotcoolant != null && ic2coolant != null) {
            //println("IC2 Coolant Enabled in Thermal Heat Pump")
            thermalPairs.add(ThermalPairing(ic2coolant, ic2hotcoolant, -640.0 / 7.0, 9, 1.0, false, minTemp = -100.0))
            thermalPairs.add(ThermalPairing(ic2hotcoolant, ic2coolant, 640.0 / 7.0, 9, 1.0, false))
        }

        if (steam != null) {
            //println("Steam Enabled in Thermal Heat Pump")
    ...
    override fun initialize() {
        (descriptor as ThermalHeatPumpDescriptor).thermal.applyTo(thermalLoad)
        connect()
    }
...
class ThermalHeatPumpRender(
    tileEntity: TransparentNodeEntity,
    descriptor: TransparentNodeDescriptor
): TransparentNodeElementRender(tileEntity, descriptor) {
    override fun draw() {
        front!!.glRotateXnRef()
        (transparentNodedescriptor as ThermalHeatPumpDescriptor).draw()
    }
}
