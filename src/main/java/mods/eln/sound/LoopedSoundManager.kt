    ...
    fun process(deltaT: Float) {
        ...
        if (remaining <= 0) {
            ...
            loops.forEach {
                ...
                if (it.volume > 0 && it.pitch > 0 && !soundHandler.isSoundPlaying(it) && distDeltaSquared < Vars.maxSoundDistance * Vars.maxSoundDistance) {
                    try {
                ...
                if (distDeltaSquared >= Vars.maxSoundDistance * Vars.maxSoundDistance || it.volume == 0f || it.pitch == 0f) {
                    try {
